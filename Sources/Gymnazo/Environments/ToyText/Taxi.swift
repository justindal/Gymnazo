...
public final class Taxi: Env {
                ...
                for passIdx in 0..<5 {
                    for destIdx in 0..<4 {
                        let state = Self.encode(
                            taxiRow: row, taxiCol: col, passLoc: passIdx, destIdx: destIdx)
                        ...
                        for action in 0..<numActions {
                            if isRainy {
                                P[state][action] = buildRainyTransitions(
                                    row: row, col: col, passIdx: passIdx, destIdx: destIdx,
                                    action: action
                                )
                            } else {
    ...
    public static func decode(_ state: Int) -> (
        taxiRow: Int, taxiCol: Int, passLoc: Int, destIdx: Int
    ) {
        var i = state
        let destIdx = i % 4
        i = i / 4
    ...
    private func pickup(taxiLoc: (Int, Int), passIdx: Int, reward: Double) -> (
        newPassIdx: Int, newReward: Double
    ) {
    ...
    private func dropoff(taxiLoc: (Int, Int), passIdx: Int, destIdx: Int, defaultReward: Double)
        -> (newPassIdx: Int, newReward: Double, terminated: Bool)
    {
    ...
    private func buildDryTransitions(row: Int, col: Int, passIdx: Int, destIdx: Int, action: Int)
        -> [Transition]
    {
        let taxiLoc = (row, col)
        var newRow = row
        ...
        case 5:
            let result = dropoff(
                taxiLoc: taxiLoc, passIdx: newPassIdx, destIdx: destIdx, defaultReward: reward)
            newPassIdx = result.newPassIdx
            reward = result.newReward
        ...
        let newState = Self.encode(
            taxiRow: newRow, taxiCol: newCol, passLoc: newPassIdx, destIdx: destIdx)
        return [(1.0, newState, reward, terminated)]
    }

    private func calcNewPosition(row: Int, col: Int, movement: (Int, Int), checkEast: Bool) -> (
        Int, Int
    ) {
        let (dr, dc) = movement
    ...
    private func buildRainyTransitions(row: Int, col: Int, passIdx: Int, destIdx: Int, action: Int)
        -> [Transition]
    {
        let taxiLoc = (row, col)
        var newRow = row
        ...
        if action <= 3 {
            ...
            if canMove, let (intended, left, right) = moves[action] {
                ...
                let leftPos = calcNewPosition(
                    row: row, col: col, movement: left, checkEast: left.1 > 0)
                let rightPos = calcNewPosition(
                    row: row, col: col, movement: right, checkEast: right.1 > 0)

                let intendedState = Self.encode(
                    taxiRow: newRow, taxiCol: newCol, passLoc: newPassIdx, destIdx: destIdx)
                let leftState = Self.encode(
                    taxiRow: leftPos.0, taxiCol: leftPos.1, passLoc: newPassIdx, destIdx: destIdx)
                let rightState = Self.encode(
                    taxiRow: rightPos.0, taxiCol: rightPos.1, passLoc: newPassIdx, destIdx: destIdx)
                ...
                let state = Self.encode(
                    taxiRow: row, taxiCol: col, passLoc: newPassIdx, destIdx: destIdx)
    ...
    private func _renderText() -> String {
        ...
        if out[1 + di][2 * dj + 1] != "T" && out[1 + di][2 * dj + 1] != "@"
            && out[1 + di][2 * dj + 1] != "P"
        {
            out[1 + di][2 * dj + 1] = "D"
        }
        ...
        private static func renderGUI(snapshot: TaxiRenderSnapshot, mode: RenderMode) -> CGImage? {
            ...
            case .human:
                ...
                #else
                    print(
                        "[Gymnazo] SwiftUI Canvas available via TaxiCanvasView; integrate it into your app UI."
                    )
                #endif
                return nil
            case .rgbArray:
    ...
    public struct TaxiCanvasView: View {
        ...
        public var body: some View {
            ...
            .frame(
                width: CGFloat(gridCols) * cellSize + 40, height: CGFloat(gridRows) * cellSize + 60)
        }
        ...
        private func drawGrid(context: inout GraphicsContext, size: CGSize) {
            ...
            for row in 0..<gridRows {
                for col in 0..<gridCols {
                    let origin = cellOrigin(row: row, col: col)
                    let rect = CGRect(
                        origin: origin, size: CGSize(width: cellSize, height: cellSize))
        ...
        private func drawDestination(context: inout GraphicsContext) {
            ...
            flagPath.addLine(
                to: CGPoint(x: center.x + 10 + flagSize, y: center.y + 15 - poleHeight + 8))
        ...
        private func drawPassenger(context: inout GraphicsContext) {
            if snapshot.passIdx < 4 {
                ...
                drawPersonShape(
                    context: &context, at: CGPoint(x: center.x - 12, y: center.y), scale: 1.0,
                    color: .cyan)
            }
        }

        private func drawPersonShape(
            context: inout GraphicsContext, at point: CGPoint, scale: CGFloat, color: Color
        ) {
        ...
        private func drawTaxi(context: inout GraphicsContext) {
            ...
            context.fill(
                Path(roundedRect: windowRect, cornerRadius: 2), with: .color(Color(white: 0.8)))

            let leftWheelCenter = CGPoint(
                x: center.x - bodyWidth / 3, y: center.y + bodyHeight / 2 + 2)
            let rightWheelCenter = CGPoint(
                x: center.x + bodyWidth / 3, y: center.y + bodyHeight / 2 + 2)

            let leftWheelPath = Path(
                ellipseIn: CGRect(
