...
public final class CliffWalking: Env {
    ...
    public init(
        ...
        self.cliff = Array(
            repeating: Array(repeating: false, count: Self.numCols), count: Self.numRows)
    ...
    #if canImport(SwiftUI)
        @MainActor
        private static func renderGUI(snapshot: CliffWalkingRenderSnapshot, mode: RenderMode)
            -> CGImage?
        {
            ...
            case .human:
                ...
                #else
                    print(
                        "[Gymnazo] SwiftUI Canvas available via CliffWalkingCanvasView; integrate it into your app UI."
                    )
                #endif
                return nil
            case .rgbArray:
    ...
    public struct CliffWalkingCanvasView: View {
        ...
        public var body: some View {
            ...
            .frame(
                width: CGFloat(numCols) * cellWidth + 40, height: CGFloat(numRows) * cellHeight + 40
            )
        }
        ...
        private func drawGrid(context: inout GraphicsContext) {
            for row in 0..<numRows {
                for col in 0..<numCols {
                    let origin = cellOrigin(row: row, col: col)
                    let rect = CGRect(
                        origin: origin, size: CGSize(width: cellWidth, height: cellHeight))
                    ...
                    context.stroke(
                        Path(rect), with: .color(Color(white: 0.3, opacity: 0.3)),
        ...
        private func drawCliff(context: inout GraphicsContext) {
            for col in 1..<(numCols - 1) {
                ...
                context.drawLayer { layerContext in
                    ...
                    layerContext.stroke(
                        stripePath, with: .color(Color.red.opacity(0.4)),
