...
public struct Acrobot: Env {
    ...
    private func dsdt(_ sAugmented: [Float]) -> [Float] {
        ...
        let phi1 = -m2 * l1 * lc2 * dtheta2 * dtheta2 * sin(theta2)
        ...
        if bookOrNips == "nips" {
            ...
            // Sutton & Barto
            ddtheta2 = (a + d2 / d1 * phi1 - m2 * l1 * lc2 * dtheta1 * dtheta1 * sin(theta2) - phi2)
...
public class AcrobotScene: SKScene {
    ...
    private func updatePositions() {
        ...
        // Update link 1
        updateLinkShape(link1, from: origin, to: p1Screen, length: CGFloat(snapshot.linkLength1) * scale)
        
        // Update link 2
        updateLinkShape(link2, from: p1Screen, to: p2Screen, length: CGFloat(snapshot.linkLength2) * scale)
    ...
    private func updateLinkShape(_ link: SKShapeNode?, from start: CGPoint, to end: CGPoint, length: CGFloat) {
