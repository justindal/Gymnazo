...
public final class RoadTile {
    ...
    public init(
        idx: Int, roadFriction: Float, color: (Float, Float, Float), vertices: [(Float, Float)]
    ) {
        self.idx = idx
...
public struct TrackGenerator {
    ...
    public static func createTrack(
        ...
        let noiseArray = MLX.uniform(
            low: 0, high: Float(2 * Float.pi / Float(TrackConstants.checkpoints)),
            [TrackConstants.checkpoints], key: checkpointKey)
        let (radKey, nextKey2) = MLX.split(key: key)
        key = nextKey2
        let radArray = MLX.uniform(
            low: TrackConstants.trackRad / 3, high: TrackConstants.trackRad,
            [TrackConstants.checkpoints], key: radKey)
        ...
        for i in 0..<slicedTrack.count {
            ...
            let road1L = (
                x1 - TrackConstants.trackWidth * cos(beta1),
                y1 - TrackConstants.trackWidth * sin(beta1)
            )
            let road1R = (
                x1 + TrackConstants.trackWidth * cos(beta1),
                y1 + TrackConstants.trackWidth * sin(beta1)
            ...
            let road2L = (
                x2 - TrackConstants.trackWidth * cos(beta2),
                y2 - TrackConstants.trackWidth * sin(beta2)
            )
            let road2R = (
                x2 + TrackConstants.trackWidth * cos(beta2),
                y2 + TrackConstants.trackWidth * sin(beta2)
