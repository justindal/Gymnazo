...
public struct LunarLanderContinuous: Env {
    ...
    public mutating func step(_ action: MLXArray) throws -> Step {
        ...
        if enableWind && !leftLegContact && !rightLegContact {
            let windMag =
                tanh(sin(0.02 * Float(windIdx)) + sin(Float.pi * 0.01 * Float(windIdx))) * windPower
            windIdx += 1

            let torqueMag =
                tanh(sin(0.02 * Float(torqueIdx)) + sin(Float.pi * 0.01 * Float(torqueIdx)))
                * turbulencePower
            torqueIdx += 1
        ...
        if mainAction > 0.0 {
            ...
            let ox =
                tip.0 * (Self.mainEngineYLocation / Self.scale + 2 * dispersion0) + side.0
                * dispersion1
            let oy =
                -tip.1 * (Self.mainEngineYLocation / Self.scale + 2 * dispersion0) - side.1
                * dispersion1
            ...
            b2Body_ApplyLinearImpulse(
                landerId, b2Vec2(x: impulseX, y: impulseY), impulsePoint, true)
        }
        ...
        if abs(lateralAction) > 0.5 {
            ...
            let ox =
                tip.0 * dispersion0 + side.0
                * (3 * dispersion1 + direction * Self.sideEngineAway / Self.scale)
            let oy =
                -tip.1 * dispersion0 - side.1
                * (3 * dispersion1 + direction * Self.sideEngineAway / Self.scale)
            ...
            b2Body_ApplyLinearImpulse(
                landerId, b2Vec2(x: impulseX, y: impulseY),
                b2Vec2(x: impulsePointX, y: impulsePointY), true)
        }
        ...
        let shaping =
            -100.0 * sqrt(stateArray[0] * stateArray[0] + stateArray[1] * stateArray[1])
    ...
    private mutating func createLegs() {
        ...
        for i in [-1, 1] {
            ...
            jointDef.localAnchorB = b2Vec2(
                x: sign * Self.legAway / Self.scale, y: Self.legDown / Self.scale)
    ...
    private func checkBodyGroundContact(_ bodyId: b2BodyId) -> Bool {
        ...
        for i in 0..<Int(count) {
            ...
            if filterA.categoryBits == Self.groundCategory
                || filterB.categoryBits == Self.groundCategory
            {
                return true
            }
        }
