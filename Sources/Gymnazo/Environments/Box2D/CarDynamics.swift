...
public struct Car {
    ...
    private static func createHullFixtures(hullId: b2BodyId) {
        ...
        for poly in polys {
            var verts = poly.map {
                b2Vec2(x: $0.0 * CarConstants.size, y: $0.1 * CarConstants.size)
            }
    ...
    public mutating func step(dt: Float, tileFrictions: [Int: Float]) {
        for i in 0..<wheels.count {
            ...
            wheels[i].omega +=
                dt * CarConstants.enginePower * wheels[i].gas / CarConstants.wheelMomentOfInertia
                / (abs(wheels[i].omega) + 5.0)
            ...
            wheels[i].omega -=
                dt * fForce * wheels[i].wheelRadius / CarConstants.wheelMomentOfInertia
